  ...
  return (
    <div className={`codec-portrait ${side} portrait-emotion-${emotion} ${enabled && speaking ? 'is-speaking' : ''}`}>
      ...
      <span className={`codec-portrait-avatar expression-${expression}`}>
        {image ? <img src={image} alt="" data-system-portrait={image.includes('/portraits/system/') ? 'true' : undefined} data-character-portrait={image.includes('/portraits/mgs1/') ? 'true' : undefined} /> : initials}
