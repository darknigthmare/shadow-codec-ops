...
## Completed MGS2/MGS3 portrait expansion, VR Character Pack 01 and Playable Operatives Pack 02
...
Playable Operatives Pack 02 adds eight transparent 32 x 48 RGBA sprites under `public/sideops/characters/`: Solid Snake (MG1 and MG2), Raiden (MGS2), Naked Snake (MGS3), Old Snake (MGS4), Big Boss (Peace Walker and Ground Zeroes) and Venom Snake (MGSV). Mission Builder missions now resolve the playable sprite from both `era` and `mainCharacter`; choosing the `vr` environment reuses VR Character Pack 01 for the player, guards, reinforcements and boss.

As with the MGS1 and previous Side Ops packs, all delivered visuals are original fan-made creations, not extracted game assets. The eight Pack 02 sprites were generated by OpenAI from official Konami character references; the exact aliases, asset paths and source pages are documented in `CODEC_DRAWERS_AND_AVATARS_REPORT.md`.
...
- Seize sprites Side Ops originaux sous `public/sideops/characters/` : le pack de base (deux Snake, quatre gardes/renforts et deux boss) et huit personnages jouables couvrant MG1, MG2, MGS2, MGS3, MGS4, Peace Walker, Ground Zeroes et MGSV
...
- résolution du sprite jouable selon la combinaison `era` + `mainCharacter`, avec réutilisation du pack de personnages VR lorsque l’environnement `vr` est sélectionné ;
...
Les conversations, tapes et missions fournies sont des textes originaux de simulation, conçus pour relier le Codec, Side Ops, VR Missions, Mother Base et les futurs modules sans inclure d’assets officiels. Les portraits Codec et les sprites Side Ops 2D intégrés sont des créations originales fan-made inspirées de références officielles, jamais des extractions ; Playable Operatives Pack 02 a été généré par OpenAI à partir de pages officielles Konami. Les autres éléments visuels restent générés par Phaser/CSS. Aucun asset officiel n'est inclus.
